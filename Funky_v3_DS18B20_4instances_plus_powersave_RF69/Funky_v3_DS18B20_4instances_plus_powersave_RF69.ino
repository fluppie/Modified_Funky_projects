//--------------------------------------------------------------------------------------
// Measure temperature using DS18B20 sensor(s) attached to Funky v3
// harizanov.com
// GNU GPL V3
//--------------------------------------------------------------------------------------
...
#define ONE_WIRE_BUS_A 11   // The data pin, digital 11
#define ONE_WIRE_BUS_B 8    // The data pin, digital 8
#define ONE_WIRE_BUS_C 7    // The data pin, digital 7
#define ONE_WIRE_BUS_D 6    // The data pin, digital 6
...
//addresses of sensors, max. 3 sensors per "line" => 12 temperature values sending limit
byte allAddressA [3][8];  // 8 bytes per address
byte allAddressB [3][8];  // 8 bytes per address
byte allAddressC [3][8];  // 8 bytes per address
byte allAddressD [3][8];  // 8 bytes per address
...
//########################################################################################################################
 ...
 typedef struct {
	...
	int temp1;	// Temperature 1 reading
  	int temp2;	// Temperature 2 reading
  	int temp3;	// Temperature 3 reading
  	int temp4;	// Temperature 4 reading
    	int temp5;	// Temperature 5 reading
  	int temp6;	// Temperature 6 reading
  	int temp7;	// Temperature 7 reading
    	int temp8;	// Temperature 8 reading
      	int temp9;	// Temperature 9 reading
  	int temp10;	// Temperature 10 reading
    	int temp11;	// Temperature 11 reading
        int temp12;	// Temperature 12 reading		
 } Payload;
...
//      clock_prescale_set(clock_div_1);   //Run at 4Mhz so we can talk to the RFM12B over SPI
  ...
  //rf12_control(0xC000);                 // Commenting this out because sketch failed to compile with RF69 compat
  ...
  dodelay(250); // Allow 250ms for the sensors to be ready
  
  // Start up the library
  sensorsA.begin(); 
  sensorsA.setWaitForConversion(false);   
  numSensorsA=sensorsA.getDeviceCount();
  
  sensorsB.begin(); 
  sensorsB.setWaitForConversion(false);   
  numSensorsB=sensorsB.getDeviceCount();
  
  sensorsC.begin(); 
  sensorsC.setWaitForConversion(false);   
  numSensorsC=sensorsC.getDeviceCount();
  
  sensorsD.begin(); 
  sensorsD.setWaitForConversion(false);   
  numSensorsD=sensorsD.getDeviceCount(); 
 
  byte j=0;                                        // search for one wire devices and
                                                   // copy to device address arrays.
  while ((j < numSensorsA) && (oneWireA.search(allAddressA[j]))) {        
    j++;
    
    //pretty much useless now since we power the DS18b20 on and off via a digital pin, and the power on default is 12bit
    sensorsA.setResolution(allAddressA[j], TEMPERATURE_PRECISION);      // and set the a to d conversion resolution of each.
  }
 
  byte k=0;                                        // search for one wire devices and
                                                   // copy to device address arrays.
  while ((k < numSensorsB) && (oneWireB.search(allAddressB[k]))) {        
    k++;
    
    //pretty much useless now since we power the DS18b20 on and off via a digital pin, and the power on default is 12bit
    sensorsB.setResolution(allAddressB[k], TEMPERATURE_PRECISION);      // and set the a to d conversion resolution of each.
  }
  
  byte l=0;                                        // search for one wire devices and
                                                   // copy to device address arrays.
  while ((l < numSensorsC) && (oneWireC.search(allAddressC[l]))) {        
    l++;
    
    //pretty much useless now since we power the DS18b20 on and off via a digital pin, and the power on default is 12bit
    sensorsC.setResolution(allAddressC[l], TEMPERATURE_PRECISION);      // and set the a to d conversion resolution of each.
  }
 
  byte m=0;                                        // search for one wire devices and
                                                   // copy to device address arrays.
  while ((m < numSensorsD) && (oneWireD.search(allAddressD[m]))) {        
    m++;
    
    //pretty much useless now since we power the DS18b20 on and off via a digital pin, and the power on default is 12bit
    sensorsD.setResolution(allAddressD[m], TEMPERATURE_PRECISION);      // and set the a to d conversion resolution of each.
  }  
    
  if(usb==1) { 
    Serial.print("NumSensorsA:"); Serial.println(numSensorsA);
    Serial.print("NumSensorsB:"); Serial.println(numSensorsB); 
    Serial.print("NumSensorsC:"); Serial.println(numSensorsC);
    Serial.print("NumSensorsD:"); Serial.println(numSensorsD); 

  for (byte j=0; j < numSensorsA; j++) {
    Serial.print("Device A ");
    Serial.print(j);  
    Serial.print(": ");                          
    printAddress(allAddressA[j]);                  // print address from each device address arry.
  }
  
  for (byte k=0; k < numSensorsB; k++) {
    Serial.print("Device B ");
    Serial.print(k);  
    Serial.print(": ");                          
    printAddress(allAddressB[k]);                  // print address from each device address arry.
  }
  
  for (byte l=0; l < numSensorsC; l++) {
    Serial.print("Device C ");
    Serial.print(l);  
    Serial.print(": ");                          
    printAddress(allAddressC[l]);                  // print address from each device address arry.
  }
  
  for (byte m=0; m < numSensorsD; m++) {
    Serial.print("Device D ");
    Serial.print(m);  
    Serial.print(": ");                          
    printAddress(allAddressD[m]);                  // print address from each device address arry.
  }
...
void loop() {
  ...
  for(int j=0;j<numSensorsA;j++) {
    sensorsA.setResolution(allAddressA[j], TEMPERATURE_PRECISION);      // and set the a to d conversion resolution of each.
  }
  
  for(int k=0;k<numSensorsB;k++) {
    sensorsB.setResolution(allAddressB[k], TEMPERATURE_PRECISION);      // and set the a to d conversion resolution of each.
  }

  for(int l=0;l<numSensorsC;l++) {
    sensorsC.setResolution(allAddressC[l], TEMPERATURE_PRECISION);      // and set the a to d conversion resolution of each.
  }
  
  for(int m=0;m<numSensorsD;m++) {
    sensorsD.setResolution(allAddressD[m], TEMPERATURE_PRECISION);      // and set the a to d conversion resolution of each.
  }
...
//                       temptx.temp1=(sensorsA.getTempC(allAddressA[0])*100);   if(usb==1) { Serial.print("Temperature A 1:"); Serial.println(temptx.temp1);}
//  if (numSensorsA>1) { temptx.temp2=(sensorsA.getTempC(allAddressA[1])*100);   if(usb==1) { Serial.print("Temperature A 2:"); Serial.println(temptx.temp2);} }
//  if (numSensorsA>2) { temptx.temp3=(sensorsA.getTempC(allAddressA[2])*100);   if(usb==1) { Serial.print("Temperature A 3:"); Serial.println(temptx.temp3);} }
//  
//                       temptx.temp4=(sensorsB.getTempC(allAddressB[0])*100);   if(usb==1) { Serial.print("Temperature B 1:"); Serial.println(temptx.temp4);}
//  if (numSensorsB>1) { temptx.temp5=(sensorsB.getTempC(allAddressB[1])*100);   if(usb==1) { Serial.print("Temperature B 2:"); Serial.println(temptx.temp5);} }
//  if (numSensorsB>2) { temptx.temp6=(sensorsB.getTempC(allAddressB[2])*100);   if(usb==1) { Serial.print("Temperature B 3:"); Serial.println(temptx.temp6);} }
//  if (numSensorsB>3) { temptx.temp7=(sensorsB.getTempC(allAddressB[3])*100);   if(usb==1) { Serial.print("Temperature B 4:"); Serial.println(temptx.temp7);} }
  ...
  if (numSensorsB>2) { temptx.temp6=(sensorsB.getTempC(allAddressB[2])*100);   if(usb==1) { Serial.print("Temperature B 3:"); Serial.println(temptx.temp6);} }
                       ...
                       temptx.temp7=(sensorsC.getTempC(allAddressC[0])*100);   if(usb==1) { Serial.print("Temperature C 1:"); Serial.println(temptx.temp7);}
  if (numSensorsC>1) { temptx.temp8=(sensorsC.getTempC(allAddressC[1])*100);   if(usb==1) { Serial.print("Temperature C 2:"); Serial.println(temptx.temp8);} }
  if (numSensorsC>2) { temptx.temp9=(sensorsC.getTempC(allAddressC[2])*100);   if(usb==1) { Serial.print("Temperature C 3:"); Serial.println(temptx.temp9);} }
  
                       temptx.temp10=(sensorsD.getTempC(allAddressD[0])*100);   if(usb==1) { Serial.print("Temperature D 1:"); Serial.println(temptx.temp10);}
  if (numSensorsD>1) { temptx.temp11=(sensorsD.getTempC(allAddressD[1])*100);   if(usb==1) { Serial.print("Temperature D 2:"); Serial.println(temptx.temp11);} }
  if (numSensorsD>2) { temptx.temp12=(sensorsD.getTempC(allAddressD[2])*100);   if(usb==1) { Serial.print("Temperature D 3:"); Serial.println(temptx.temp12);} }
...
static void rfwrite(){
      ...
      if(storage.ACK) {
         for (byte i = 0; i <= RETRY_LIMIT; ++i) {  // tx and wait for ack up to RETRY_LIMIT times
           ...
           while (!rf12_canSend())
              rf12_recvDone();
           //rf12_sendStart(RF12_HDR_ACK, &temptx, (numSensorsA+numSensorsB+numSensorsC+numSensorsD)*2 + 2); 
           rf12_sendStart(RF12_HDR_ACK, &temptx, (12)*2 + 2); 
      ...
      while (!rf12_canSend())
        rf12_recvDone();
      //rf12_sendStart(0, &temptx, (numSensorsA+numSensorsB+numSensorsC+numSensorsD)*2 + 2);
      rf12_sendStart(0, &temptx, (12)*2 + 2);  
           if (usb==0) rf12_sendWait(2);           // Wait for RF to finish sending while in standby mode
             else rf12_sendWait(0); 
      rf12_sleep(0);              // Put RF module to sleep 
      power_spi_disable();      
